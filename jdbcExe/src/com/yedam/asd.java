...
class Board {
	...
	void doCommandList() {
		...
		System.out.printf("번호 | 조회수 |          날짜          |      제목      | 작성자\n");
		if (articlesLastIndex >= 0) {
			for (int i = 0; i <= articlesLastIndex; i++) {
				System.out.printf("%d    | %d      | %s    | %s    | %s\n", articles[i].id, articles[i].views_count,
						articles[i].resDate, articles[i].title, articles[i].writer);
			}
		} else {
	...
	void doCommandAdd() {
		...
		// ex) 2짜리 배열의 경우 0, 1번에 값 저장, 마지막 배열인 1과 배열 길이 2 - 1 이 같은 경우 배열 확장.
